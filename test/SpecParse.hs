...
sampleSpiral =
  describe "Spiral Sample FITS Parse" $ do
    it "should parse" $ do
      ...
      let payloadSize = BL.length (hdu ^. mainData)

      -- Make sure we took the right number of bytes out of the file
      payloadSize @?= fileSizeOnDisk - fromIntegral hduBlockSize
...
sampleNSOHeaders = do
  describe "NSO Stripped Headers" $ do
    ...
    describe "sample header file" $ do
      bs <- liftIO $ BL.readFile "./fits_files/nso_dkist_headers.txt"
      let ts = filter (not . ignore) $ TL.lines $ TE.decodeUtf8 bs
  ...
  where
    ignore t = TL.isPrefixOf "CONTINUE" t || TL.isPrefixOf "END" t
  ...
  describe "NSO Sample FITS Parse" $ do
    ...
    it "should parse both HDUs" $ do
      ...
      let sizeOnDisk = 161280
          ...
          payloadLength = BL.length (h2 ^. mainData)
      ...
      assertBool "The end of the heap has some null data" $ do
        C8.all (/='\0') $ C8.take 100 $ C8.drop (fromIntegral headerLength + payloadLength + fromIntegral heapLength - 100) bs
      assertBool "The remainder of the file contains real data" $ do
        C8.all (=='\0') $ C8.drop (fromIntegral headerLength + payloadLength + fromIntegral heapLength) bs
      where
